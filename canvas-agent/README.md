...
Debug 日志会以 `[DEBUG][HH:mm:ss]` 等传统格式输出到终端，并按启动日期保存到 `~/.nova-canvas/logs/canvas-agent-YYYY-MM-DD.log`。终端日志带级别颜色，文件日志为纯文本；日志包含 HTTP、SSE、线程、turn、Codex app-server 和工具调用事件，token 与图片 Data URL 会自动隐藏。
...
Canvas Agent 默认只监听 `127.0.0.1`。网页第一次带正确 token 连接后，Canvas Agent 会记录该网页 Origin；之后其他 Origin 不能复用这个本地 Agent，除非用户清理 `~/.nova-canvas/canvas-agent.json` 里的 `origins`。
...
直接运行 `npx -y @basketikun/canvas-agent` 只启动本地 Agent 服务，不会安装 MCP，也不会增加 Codex 工具上下文。只有安装 Codex app 插件，或手动执行 `codex mcp add` 后，`nova-canvas` 工具才会进入 Codex 上下文；由于工具较多，不使用时建议移除。

通过插件安装时移除插件：

```bash
codex plugin remove nova-canvas
```

手动添加 MCP 时移除 MCP：

```bash
codex mcp remove nova-canvas
```

### Codex app 插件

仓库内提供了 Codex app 插件：`plugins/nova-canvas`。在 Codex app 中添加本仓库的 marketplace 后，可以安装 `Nova Canvas` 插件；插件会注册同一个 `nova-canvas` MCP，并带上画布操作说明。
...
使用时可以直接在 Codex 里说“打开 Nova Canvas”，插件会启动本地 Agent，读取 Local URL 和 Connect token，然后在右侧打开 `https://canvas.best/` 并自动新建、连接画布；只有明确要求使用本地项目时才会启动本地前端。

Canvas Agent 启动后，给 Codex 添加 MCP：

```bash
codex mcp add nova-canvas -- npx -y @basketikun/canvas-agent mcp
```

本仓库开发时可以改成，实际使用建议替换为本机绝对路径：

```bash
codex mcp add nova-canvas -- node /path/to/nova-canvas/canvas-agent/dist/index.js mcp
```
...
本地面板会把提示词发送给 Canvas Agent。Canvas Agent 使用官方 `@openai/codex` CLI 的 `codex app-server --stdio` 启动并复用同一个 Codex thread，启动时会注入 `nova-canvas` MCP 配置并自动放行 MCP 审批，真正执行画布修改前仍由网页侧边栏二次确认。
...
```bash
claude mcp add --scope user --transport stdio nova-canvas -- npx -y @basketikun/canvas-agent mcp
```

本仓库开发时可以改成：

```bash
claude mcp add --scope user --transport stdio nova-canvas -- node /path/to/nova-canvas/canvas-agent/dist/index.js mcp
```

Canvas Agent 调用 Claude Code 时会默认带上 `--allowedTools mcp__nova-canvas__*`，画布写操作仍由网页侧边栏确认。
