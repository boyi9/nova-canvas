...
- 画布项目 JSON：`localForage`，数据库名 `nova-canvas`，storeName `app_state`，key 为 `nova-canvas:canvas_store`。
- 我的素材 JSON：`localForage`，数据库名 `nova-canvas`，storeName `app_state`，key 为 `nova-canvas:asset_store`。
- 图片 Blob：单独存到 `localForage` 实例，数据库名 `nova-canvas`，storeName `image_files`。
- 视频等媒体 Blob：单独存到 `localForage` 实例，数据库名 `nova-canvas`，storeName `media_files`。
